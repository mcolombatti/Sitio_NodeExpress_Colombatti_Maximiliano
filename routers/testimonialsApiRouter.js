...
router.route('/')
    ...
    router.route('/:id')
    .delete([mw_testimonial], function(req, res){
        promises.readFile('./data/testimonials.json')
        .then(function(data){
            ...
            if(testimonial){
                ...
                promises.writeFile('./data/testimonials.json', JSON.stringify(testimonials))
                .then(function(){
        ...
        .catch(function(err){
            res.status(404).json({ err:404, msg: `no se encuentra el testimonio id: ${req.params.id}` } )
        })
    })
    .get(function (req, res) {
        promises.readFile('./data/testimonials.json')
            .then(function (data) {
                ...
                if (tst) {
                    if (tst.deleted != true) {
                        ...
                        res.status(404).json({ err: 404, msg: `El testimonio #${req.params.id} fue eliminado.` })
                    }
                }
                else {
